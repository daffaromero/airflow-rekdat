...
csv_path = Path("/opt/airflow/data/tweets_crawl_new.csv")
...
def addtodb():
    ...
    cursor = conn.cursor()
    cursor.execute("""
            CREATE TABLE IF NOT EXISTS tweets_crawl_new (
                twitId varchar(100),
    ...
    with open('/opt/airflow/data/tweets_crawl_new.csv', 'r') as f:
        reader = csv.reader(f)
        next(reader)
        for row in reader:
            cursor.execute(
                "INSERT INTO tweets_crawl_new VALUES (%s, %s, %s)",
                row
            )
    conn.commit()
